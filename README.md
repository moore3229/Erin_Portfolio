...
- Built a Pie Chart  breaking % of Spending by Category.
...
Used artificial neural network(s) to solve simple regression problem. I fitted a neural network to a 3D sinusoidal data set. Used a Sequential model to be trained very quickly on the supplied data, and manually adjusted hyperparameter values and observed their influence on the model's predictions.

Built an ANN that modeled the data. Trained the model and used it to predict a smooth and less eratic curve. Computed the generalization error on the test set.
